...
class GlObject {
  ...
  void drawElements(int mode, [int length = -1, int offset = 0]) {
    ...
    final len = length == -1 ? indexBuffer.size : length;
    _ctx.drawElements(mode, len, GlIndexBuffer.indexBufferType, offset);
  }
  ...
  void drawElementsInstanced(int mode, int count, Map<String, int> divisors,
    ...
    // Get ANGLE extension.
    final gl.AngleInstancedArrays ext =
        _ctx.getExtension('ANGLE_instanced_arrays');
    ...
    ext.drawElementsInstancedAngle(
        mode, len, GlIndexBuffer.indexBufferType, offset, count);
  }
}
