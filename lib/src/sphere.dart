...
class GlSphere extends GlGeometry {
  ...
  static Vector3List generateSpherePositions(
    ...
    final v = new Vector3List((phiSegments - 1) * thetaSegments + 2);
  ...
  static Uint16List generateWireframeIndices(
    ...
    for (var phi = 1; phi < phiSegments; phi++) {
      for (var theta = 0; theta < thetaSegments; theta++) {
        final idx = 2 + (phi - 1) * thetaSegments + theta;
  ...
  static Uint16List generateSurfaceIndices(int phiSegments, int thetaSegments) {
    final t = new Uint16List((phiSegments - 1) * thetaSegments * 6);
    var i = 0;
    ...
    for (var phi = 1; phi < phiSegments; phi++) {
      for (var theta = 0; theta < thetaSegments; theta++) {
        final idx = 2 + (phi - 1) * thetaSegments + theta;
        final prev = theta == 0 ? idx + thetaSegments - 1 : idx - 1;
        final up = phi == 1 ? 0 : idx - thetaSegments;
        final down = phi == phiSegments - 1 ? 1 : prev + thetaSegments;
